	...
	//		  - v2.3 : Non integral inputs avoided (See issues) and comments added
	...
	int main(void){
	    ...
	    int user_input;						// Integral value entered by user
	    long long out;						// To store output
	    cout<<"Please enter an integer between (including) 1 and 200 : ";
	    cin>>user_str;						// Read input from user
	    const char *char_input = user_str.c_str();			// Convert string entered by user to character pointer for use in atoi()
	    user_input = atoi(char_input);				// Converting to integer
